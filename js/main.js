...
buttonGen.addEventListener("click", function () {
    ...
    const minVagon = 1;
    const maxVagon = 10;
    let randomNumVagon = Math.floor(Math.random() * (maxVagon - minVagon + 1)) + minVagon;
    const minCode = 9999;
    const maxCode = 100000;
    let randomNumCode = Math.floor(Math.random() * (maxCode - minCode + 1)) + minCode;

    document.getElementById("vagon").innerHTML = randomNumVagon
    document.getElementById("code-cp").innerHTML = randomNumCode
    document.getElementById("user-name-tiket").innerHTML = username;
